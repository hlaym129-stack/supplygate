        ...
        <div>
          ...
          <div class="grid grid-cols-2 gap-3">
            ...
            >
              ...
              <span class="mt-1 block text-xs text-gray-500 dark:text-dark-400">使用 API、密钥和余额功能</span>
            </button>
            <button
              type="button"
              class="rounded-xl border p-3 text-left transition"
            ...
            >
              ...
              <span class="mt-1 block text-xs text-gray-500 dark:text-dark-400">提交主体资料，等待管理员审核</span>
            </button>
          </div>
        </div>

        <template v-if="isSupplierSignup">
          <div class="rounded-xl border border-gray-200 bg-gray-50 p-5 dark:border-dark-700 dark:bg-dark-800/60 space-y-4">
            <div>
              <p class="text-sm font-semibold text-gray-900 dark:text-white">供应商注册</p>
              <p class="mt-1 text-xs text-gray-500 dark:text-dark-400">填写完成后提交注册，管理员审核通过后开放账号提交功能。</p>
            </div>
            ...
            <div class="border-t border-gray-200 pt-4 dark:border-dark-700">
              <p class="text-sm font-medium text-gray-700 dark:text-dark-200 mb-3">主体资料</p>
...
const formData = reactive({
  ...
  invitation_code: '',
  aff_code: '',
  account_type: 'user' as 'user' | 'supplier',
  supplier_profile: {
    company_name: '',
    contact_name: '',
    contact_email: '',
...
const isSupplierSignup = computed(() => formData.account_type === 'supplier')
const supplierProfilePayload = computed(() => ({
  company_name: formData.supplier_profile.company_name.trim(),
  contact_name: formData.supplier_profile.contact_name.trim() || undefined,
  contact_email: formData.supplier_profile.contact_email.trim() || undefined,
  contact_phone: formData.supplier_profile.contact_phone.trim() || undefined,
  notes: formData.supplier_profile.notes.trim() || undefined
}))
const oauthSignupContext = computed(() =>
  isSupplierSignup.value
    ? { account_type: 'supplier' as const, supplier_profile: supplierProfilePayload.value }
    : { account_type: 'user' as const }
)

const showOAuthLogin = computed(
  () => {
    if (isSupplierSignup.value) {
      return linuxdoOAuthEnabled.value || wechatOAuthEnabled.value || oidcOAuthEnabled.value
    }
    return (
...
function validateForm(): boolean {
  ...
  if (isSupplierSignup.value && !supplierProfilePayload.value.company_name) {
    errors.supplier_company_name = '请输入供应商主体名称'
    isValid = false
  }
...
async function handleRegister(): Promise<void> {
  ...
  try {
    ...
    if (emailVerifyEnabled.value) {
      ...
      sessionStorage.setItem(
        ...
        JSON.stringify({
          ...
          supplier_profile: isSupplierSignup.value ? supplierProfilePayload.value : undefined,
    ...
    await router.push(defaultRedirectPath())
  } catch (error: unknown) {
