      ...
      <template #filters>
        <div class="flex flex-wrap items-center gap-3">
          ...
          <div class="flex flex-1 flex-wrap items-center gap-3">
            ...
            <div v-if="visibleFilters.has('user_type')" class="w-full sm:w-36">
              <Select
                v-model="filters.user_type"
                :options="[
                  { value: '', label: '全部类型' },
                  { value: 'regular', label: '普通用户' },
                  { value: 'supplier', label: '供应商用户' }
                ]"
                @change="applyFilter"
              />
            </div>
        ...
        >
          ...
          <template #cell-user_type="{ row }">
            <div class="flex flex-wrap items-center gap-2">
              <span :class="['badge', resolveUserType(row) === 'supplier' ? 'badge-primary' : resolveUserType(row) === 'admin' ? 'badge-purple' : 'badge-gray']">
                {{ userTypeLabel(row) }}
              </span>
              <span v-if="resolveUserType(row) === 'supplier' && row.supplier_status" class="badge badge-warning">
                {{ supplierStatusLabel(row.supplier_status) }}
              </span>
            </div>
          </template>
...
const resolveUserType = (user: AdminUser): 'admin' | 'regular' | 'supplier' => {
  if (user.user_type) return user.user_type
  if (user.role === 'admin') return 'admin'
  return user.supplier_status ? 'supplier' : 'regular'
}

const userTypeLabel = (user: AdminUser): string => {
  switch (resolveUserType(user)) {
    case 'admin':
      return '管理员'
    case 'supplier':
      return '供应商用户'
...
const supplierStatusLabel = (status?: string | null): string => {
  switch (status) {
    case 'approved':
      return '已通过'
    case 'pending':
...
const allColumns = computed<Column[]>(() => [
  ...
  { key: 'user_type', label: '用户类型', sortable: false },
...
const builtInFilters = computed(() => [
  ...
  { key: 'user_type', name: '用户类型', type: 'select' as const },
...
const loadSavedFilters = () => {
  try {
    ...
    if (savedValues) {
      ...
      if (parsed.user_type) filters.user_type = parsed.user_type
