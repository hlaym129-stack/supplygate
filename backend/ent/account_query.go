...
func (_q *AccountQuery) QuerySupplier() *SupplierProfileQuery {
...
func (_q *AccountQuery) WithSupplier(opts ...func(*SupplierProfileQuery)) *AccountQuery {
	query := (&SupplierProfileClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *AccountQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Account, error) {
	...
	if query := _q.withSupplier; query != nil {
		if err := _q.loadSupplier(ctx, query, nodes, nil,
			func(n *Account, e *SupplierProfile) { n.Edges.Supplier = e }); err != nil {
			return nil, err
		}
	}
...
func (_q *AccountQuery) loadSupplier(ctx context.Context, query *SupplierProfileQuery, nodes []*Account, init func(*Account), assign func(*Account, *SupplierProfile)) error {
	ids := make([]int64, 0, len(nodes))
	nodeids := make(map[int64][]*Account)
	for i := range nodes {
