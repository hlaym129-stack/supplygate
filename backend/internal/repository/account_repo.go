...
func (r *accountRepository) Create(ctx context.Context, account *service.Account) error {
	...
	builder := r.client.Account.Create().
		...
		SetSupportedModels(normalizeStringSlice(account.SupportedModels)).
		SetSupplierTestedModels(normalizeStringSlice(account.SupplierTestedModels)).
		SetConcurrency(account.Concurrency).
		SetPriority(account.Priority).
		SetStatus(account.Status).
		SetOwnerType(defaultAccountOwnerType(account.OwnerType)).
		SetApprovalStatus(defaultAccountApprovalStatus(account.OwnerType, account.ApprovalStatus)).
	...
	if account.ApprovedAt != nil {
		builder.SetApprovedAt(*account.ApprovedAt)
	}
	if account.ApprovedBy != nil {
		builder.SetApprovedBy(*account.ApprovedBy)
	}
	if strings.TrimSpace(account.RejectReason) != "" {
		builder.SetRejectReason(account.RejectReason)
	}
	if account.SupplierTestedAt != nil {
		builder.SetSupplierTestedAt(*account.SupplierTestedAt)
	}
	if strings.TrimSpace(account.SupplierEditRequestStatus) != "" {
		builder.SetSupplierEditRequestStatus(account.SupplierEditRequestStatus)
	}
	if strings.TrimSpace(account.SupplierEditRequestReason) != "" {
		builder.SetSupplierEditRequestReason(account.SupplierEditRequestReason)
	}
	if strings.TrimSpace(account.SupplierEditRequestReviewNote) != "" {
		builder.SetSupplierEditRequestReviewNote(account.SupplierEditRequestReviewNote)
	}
	if account.SupplierEditRequestedAt != nil {
		builder.SetSupplierEditRequestedAt(*account.SupplierEditRequestedAt)
	}
	if account.SupplierEditRequestReviewedAt != nil {
		builder.SetSupplierEditRequestReviewedAt(*account.SupplierEditRequestReviewedAt)
	}
	if account.SupplierEditRequestReviewedBy != nil {
		builder.SetSupplierEditRequestReviewedBy(*account.SupplierEditRequestReviewedBy)
	}
...
func (r *accountRepository) Update(ctx context.Context, account *service.Account) error {
	...
	if account.SupplierID != nil {
		...
		builder.ClearSupplierID()
	}
	if account.ApprovedAt != nil {
		builder.SetApprovedAt(*account.ApprovedAt)
	} else {
		builder.ClearApprovedAt()
	}
	if account.ApprovedBy != nil {
		builder.SetApprovedBy(*account.ApprovedBy)
	} else {
		builder.ClearApprovedBy()
	}
	if strings.TrimSpace(account.RejectReason) != "" {
		builder.SetRejectReason(account.RejectReason)
	} else {
	...
	if account.SupplierTestedAt != nil {
		...
		builder.ClearSupplierTestedAt()
	}
	if strings.TrimSpace(account.SupplierEditRequestStatus) != "" {
		builder.SetSupplierEditRequestStatus(account.SupplierEditRequestStatus)
	} else {
		builder.ClearSupplierEditRequestStatus()
	}
	if strings.TrimSpace(account.SupplierEditRequestReason) != "" {
		builder.SetSupplierEditRequestReason(account.SupplierEditRequestReason)
	} else {
		builder.ClearSupplierEditRequestReason()
	}
	if strings.TrimSpace(account.SupplierEditRequestReviewNote) != "" {
		builder.SetSupplierEditRequestReviewNote(account.SupplierEditRequestReviewNote)
	} else {
		builder.ClearSupplierEditRequestReviewNote()
	}
	if account.SupplierEditRequestedAt != nil {
		builder.SetSupplierEditRequestedAt(*account.SupplierEditRequestedAt)
	} else {
		builder.ClearSupplierEditRequestedAt()
	}
	if account.SupplierEditRequestReviewedAt != nil {
		builder.SetSupplierEditRequestReviewedAt(*account.SupplierEditRequestReviewedAt)
	} else {
		builder.ClearSupplierEditRequestReviewedAt()
	}
	if account.SupplierEditRequestReviewedBy != nil {
		builder.SetSupplierEditRequestReviewedBy(*account.SupplierEditRequestReviewedBy)
	} else {
		builder.ClearSupplierEditRequestReviewedBy()
	}
...
func (r *accountRepository) ListBySupplier(ctx context.Context, supplierID int64, params pagination.PaginationParams) ([]service.Account, *pagination.PaginationResult, error) {
	q := r.client.Account.Query().Where(dbaccount.SupplierIDEQ(supplierID))
	total, err := q.Count(ctx)
	if err != nil {
		return nil, nil, err
	}
	accounts, err := q.Order(dbent.Desc(dbaccount.FieldID)).
		Limit(params.Limit()).
...
func (r *accountRepository) ListSupplierAccounts(ctx context.Context, params pagination.PaginationParams, filters service.SupplierAccountListFilters) ([]service.Account, *pagination.PaginationResult, error) {
	q := r.client.Account.Query().Where(dbaccount.OwnerTypeEQ(service.AccountOwnerTypeSupplier))
	...
	accounts, err := q.
		WithSupplier(func(sq *dbent.SupplierProfileQuery) {
			sq.WithUser()
		}).
...
func (r *accountRepository) ListSchedulable(ctx context.Context) ([]service.Account, error) {
	...
	accounts, err := r.client.Account.Query().
		Where(
			dbaccount.StatusEQ(service.StatusActive),
			dbaccount.SchedulableEQ(true),
			supplierApprovedAccountPredicate(),
...
func (r *accountRepository) queryAccountsByGroup(ctx context.Context, groupID int64, opts accountGroupQueryOptions) ([]service.Account, error) {
	...
	if opts.approvedOnly {
		preds = append(preds, supplierApprovedAccountPredicate())
	}
	if opts.schedulable {
		now := time.Now()
		preds = append(preds,
...
func supplierApprovedAccountPredicate() dbpredicate.Account {
	return dbaccount.Or(
		dbaccount.OwnerTypeEQ(service.AccountOwnerTypePlatform),
		dbaccount.And(
...
func accountEntityToService(m *dbent.Account) *service.Account {
	...
	return &service.Account{
		...
		Status:                        m.Status,
		OwnerType:                     defaultAccountOwnerType(m.OwnerType),
		SupplierID:                    m.SupplierID,
		ApprovalStatus:                defaultAccountApprovalStatus(m.OwnerType, m.ApprovalStatus),
		ApprovedAt:                    m.ApprovedAt,
		ApprovedBy:                    m.ApprovedBy,
		RejectReason:                  derefString(m.RejectReason),
		SupportedModels:               cloneStringSlice(m.SupportedModels),
		SupplierTestedAt:              m.SupplierTestedAt,
		SupplierTestedModels:          cloneStringSlice(m.SupplierTestedModels),
		SupplierEditRequestStatus:     derefString(m.SupplierEditRequestStatus),
		SupplierEditRequestReason:     derefString(m.SupplierEditRequestReason),
		SupplierEditRequestReviewNote: derefString(m.SupplierEditRequestReviewNote),
		SupplierEditRequestedAt:       m.SupplierEditRequestedAt,
		SupplierEditRequestReviewedAt: m.SupplierEditRequestReviewedAt,
		SupplierEditRequestReviewedBy: m.SupplierEditRequestReviewedBy,
...
func defaultAccountOwnerType(value string) string {
	if strings.TrimSpace(value) == service.AccountOwnerTypeSupplier {
		return service.AccountOwnerTypeSupplier
	}
	return service.AccountOwnerTypePlatform
}

func defaultAccountApprovalStatus(ownerType string, value string) string {
	switch strings.TrimSpace(value) {
	case service.AccountApprovalStatusPending:
		return service.AccountApprovalStatusPending
	case service.AccountApprovalStatusRejected:
		return service.AccountApprovalStatusRejected
	case service.AccountApprovalStatusReturned:
		return service.AccountApprovalStatusReturned
	case service.AccountApprovalStatusApproved:
		return service.AccountApprovalStatusApproved
	default:
		if defaultAccountOwnerType(ownerType) == service.AccountOwnerTypeSupplier {
			return service.AccountApprovalStatusPending
		}
		return service.AccountApprovalStatusApproved
	}
