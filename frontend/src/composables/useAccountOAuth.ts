...
export interface AccountOAuthEndpoints {
  generateAuthUrl?: string
  generateSetupTokenUrl?: string
  exchangeCode?: string
  exchangeSetupTokenCode?: string
  cookieAuth?: string
  setupTokenCookieAuth?: string
}

export function useAccountOAuth(endpoints: AccountOAuthEndpoints = {}) {
  ...
  const generateAuthUrl = async (
    ...
    try {
      ...
      const endpoint =
        addMethod === 'oauth'
          ? (endpoints.generateAuthUrl || '/admin/accounts/generate-auth-url')
          : (endpoints.generateSetupTokenUrl || '/admin/accounts/generate-setup-token-url')
  ...
  const exchangeAuthCode = async (
    ...
    try {
      ...
      const endpoint =
        addMethod === 'oauth'
          ? (endpoints.exchangeCode || '/admin/accounts/exchange-code')
          : (endpoints.exchangeSetupTokenCode || '/admin/accounts/exchange-setup-token-code')
  ...
  const cookieAuth = async (
    ...
    try {
      ...
      const endpoint =
        addMethod === 'oauth'
          ? (endpoints.cookieAuth || '/admin/accounts/cookie-auth')
          : (endpoints.setupTokenCookieAuth || '/admin/accounts/setup-token-cookie-auth')
