...
func (s *openAIWSUsageHandlerAccountRepoStub) ListSchedulableByPlatform(_ context.Context, platform string) ([]service.Account, error) {
...
func (s *openAIWSUsageHandlerAccountRepoStub) ListSchedulableByGroupIDAndPlatform(ctx context.Context, _ int64, platform string) ([]service.Account, error) {
	return s.ListSchedulableByPlatform(ctx, platform)
}

func (s *openAIWSUsageHandlerAccountRepoStub) GetByID(_ context.Context, id int64) (*service.Account, error) {
	if s.account.ID != id {
		return nil, nil
	}
	account := s.account
...
func (s *openAIWSUsageHandlerUsageLogRepoStub) Create(_ context.Context, log *service.UsageLog) (bool, error) {
	if s.created != nil {
		s.created <- log
	}
	return true, nil
...
func (s *openAIWSUsageHandlerChannelRepoStub) ListAll(_ context.Context) ([]service.Channel, error) {
	return s.channels, nil
}

func (s *openAIWSUsageHandlerChannelRepoStub) GetGroupPlatforms(_ context.Context, groupIDs []int64) (map[int64]string, error) {
...
func runOpenAIResponsesWebSocketUsageLogCase(t *testing.T, tc openAIResponsesWSUsageLogCase) openAIResponsesWSUsageLogResult {
	...
	case <-time.After(3 * time.Second):
		t.Fatal("waiting for WebSocket usage log timed out")
	}
	...
	case <-time.After(3 * time.Second):
		t.Fatal("waiting for upstream WebSocket first frame timed out")
	}
	...
	case <-time.After(3 * time.Second):
		t.Fatal("waiting for upstream WebSocket completion timed out")
	}
