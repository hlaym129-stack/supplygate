...
type UsageLogQuery struct {
	...
	withSupplier     *SupplierProfileQuery
...
func (_q *UsageLogQuery) QuerySupplier() *SupplierProfileQuery {
...
func (_q *UsageLogQuery) WithSupplier(opts ...func(*SupplierProfileQuery)) *UsageLogQuery {
	query := (&SupplierProfileClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UsageLogQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*UsageLog, error) {
	...
	if query := _q.withSupplier; query != nil {
		if err := _q.loadSupplier(ctx, query, nodes, nil,
			func(n *UsageLog, e *SupplierProfile) { n.Edges.Supplier = e }); err != nil {
			return nil, err
		}
	}
...
func (_q *UsageLogQuery) loadSupplier(ctx context.Context, query *SupplierProfileQuery, nodes []*UsageLog, init func(*UsageLog), assign func(*UsageLog, *SupplierProfile)) error {
	ids := make([]int64, 0, len(nodes))
	nodeids := make(map[int64][]*UsageLog)
	for i := range nodes {
...
func (_q *UsageLogQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withSupplier != nil {
			_spec.Node.AddColumnOnce(usagelog.FieldSupplierID)
		}
		if _q.withGroup != nil {
