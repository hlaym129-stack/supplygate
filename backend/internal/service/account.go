...
type Account struct {
	...
	SupplierTestedAt              *time.Time
	SupplierTestedModels          []string
	SupplierEditRequestStatus     string
	SupplierEditRequestReason     string
	SupplierEditRequestReviewNote string
	SupplierEditRequestedAt       *time.Time
	SupplierEditRequestReviewedAt *time.Time
	SupplierEditRequestReviewedBy *int64
	ErrorMessage                  string
...
func (a *Account) IsModelSupported(requestedModel string) bool {
	if len(a.SupportedModels) > 0 && !modelListSupportsRequestedModel(a.SupportedModels, requestedModel) {
		return false
	}
...
func modelListSupportsRequestedModel(models []string, requestedModel string) bool {
	...
	for _, model := range models {
		...
		if normalized == requestedModel || matchWildcard(normalized, requestedModel) {
			return true
		}
	}
	return false
