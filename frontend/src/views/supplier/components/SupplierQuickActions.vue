<template>
  <div class="card">
    ...
    <div class="space-y-3 p-4">
      <button v-if="authStore.isSupplier" @click="router.push('/supplier/accounts')" class="group flex w-full items-center gap-4 rounded-xl bg-gray-50 p-4 text-left transition-all duration-200 hover:bg-gray-100 dark:bg-dark-800/50 dark:hover:bg-dark-800">
        ...
        <div class="min-w-0 flex-1">
          <p class="text-sm font-medium text-gray-900 dark:text-white">{{ t('supplier.dashboard.manageAccounts') }}</p>
          <p class="text-xs text-gray-500 dark:text-dark-400">{{ t('supplier.dashboard.manageAccountsHint') }}</p>
        </div>
      ...
      <button @click="router.push('/supplier/profile')" class="group flex w-full items-center gap-4 rounded-xl bg-gray-50 p-4 text-left transition-all duration-200 hover:bg-gray-100 dark:bg-dark-800/50 dark:hover:bg-dark-800">
        <div class="flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-xl bg-amber-100 transition-transform group-hover:scale-105 dark:bg-amber-900/30">
          <Icon name="user" size="lg" class="text-amber-600 dark:text-amber-400" />
        </div>
        <div class="min-w-0 flex-1">
          <p class="text-sm font-medium text-gray-900 dark:text-white">{{ t('supplier.dashboard.profile') }}</p>
          <p class="text-xs text-gray-500 dark:text-dark-400">{{ t('supplier.dashboard.profileHint') }}</p>
        </div>
        <Icon name="chevronRight" size="md" class="text-gray-400 transition-colors group-hover:text-amber-500 dark:text-dark-500" />
      </button>
    </div>
  </div>
</template>
