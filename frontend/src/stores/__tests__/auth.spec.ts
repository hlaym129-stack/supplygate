...
describe('useAuthStore', () => {
  ...
  describe('legacy supplier role warnings', () => {
    it('对 legacy supplier role 登录响应输出告警', async () => {
      const legacySupplierResponse = {
        ...fakeAuthResponse,
        user: {
          ...fakeUser,
      ...
      mockLogin.mockResolvedValue(legacySupplierResponse)
      const store = useAuthStore()

      await store.login({ email: 'test@example.com', password: '123456' })

      expect(store.user?.role).toBe('supplier')
      expect(console.warn).toHaveBeenCalledWith(
        expect.stringContaining('/auth/me returned legacy supplier role'),
    ...
    it('对 legacy supplier role 的 /auth/me 刷新响应输出告警', async () => {
      mockLogin.mockResolvedValue(fakeAuthResponse)
      const store = useAuthStore()
      await store.login({ email: 'test@example.com', password: '123456' })

      const legacySupplierUser = {
        ...fakeUser,
        role: 'supplier' as const,
        has_supplier_access: true,
      }
      mockGetCurrentUser.mockResolvedValue({ data: legacySupplierUser })
