...
func TestConstants_值正确(t *testing.T) {
	...
	// ClientID 从环境变量读取，测试环境变量配置
	t.Setenv(AntigravityOAuthClientIDEnv, "test-client-id.apps.googleusercontent.com")
	ClientID = os.Getenv(AntigravityOAuthClientIDEnv)
	if ClientID != "test-client-id.apps.googleusercontent.com" {
		t.Errorf("ClientID 环境变量配置不生效: got %s", ClientID)
	}

	// defaultClientSecret 从环境变量读取
	...
	if secret != "test-client-secret" {
		t.Errorf("client_secret 环境变量配置不生效: got %s", secret)
	}
