...
const OAUTH_SIGNUP_CONTEXT_KEY = 'oauth_signup_context'
...
export function storeOAuthSignupContext(context?: OAuthSignupContext): void {
  if (typeof window === 'undefined') {
    return
  }
  try {
    if (context?.account_type === 'supplier' && context.supplier_profile?.company_name?.trim()) {
      window.sessionStorage.setItem(OAUTH_SIGNUP_CONTEXT_KEY, JSON.stringify(context))
    } else {
      window.sessionStorage.removeItem(OAUTH_SIGNUP_CONTEXT_KEY)
    }
  } catch {
    // 忽略浏览器存储异常。
  }
...
export function loadOAuthSignupContext(): OAuthSignupContext {
  if (typeof window === 'undefined') {
    return {}
  }
  try {
    const raw = window.sessionStorage.getItem(OAUTH_SIGNUP_CONTEXT_KEY)
    if (!raw) return {}
    const parsed = JSON.parse(raw) as OAuthSignupContext
    return parsed?.account_type === 'supplier' ? parsed : {}
  } catch {
    return {}
  }
}
