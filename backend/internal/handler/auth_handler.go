...
func NewAuthHandler(cfg *config.Config, authService *service.AuthService, userService *service.UserService, supplierService *service.SupplierService, settingService *service.SettingService, promoService *service.PromoService, redeemService *service.RedeemService, totpService *service.TotpService) *AuthHandler {
	return &AuthHandler{
		cfg:             cfg,
...
type RegisterRequest struct {
	...
	SupplierProfile service.SupplierProfileInput `json:"supplier_profile"`
}
...
func (h *AuthHandler) Register(c *gin.Context) {
	...
	if err := h.applySupplierSignupProfile(c.Request.Context(), user, req.AccountType, req.SupplierProfile, req.InvitationCode); err != nil {
		response.ErrorFrom(c, err)
		return
	}
...
func (h *AuthHandler) applySupplierSignupProfile(ctx context.Context, user *service.User, accountType string, profile service.SupplierProfileInput, invitationCode string) error {
	if strings.TrimSpace(strings.ToLower(accountType)) != "supplier" {
		return nil
	}
	if h == nil || h.supplierService == nil || h.authService == nil || user == nil || user.ID <= 0 {
		return infraerrors.ServiceUnavailable("SUPPLIER_SIGNUP_UNAVAILABLE", "supplier signup is temporarily unavailable")
	}
	if strings.TrimSpace(profile.CompanyName) == "" {
		return infraerrors.BadRequest("SUPPLIER_COMPANY_REQUIRED", "company name is required")
	}
	if _, err := h.supplierService.ApplyProfile(ctx, user.ID, profile); err != nil {
		if rollbackErr := h.authService.RollbackOAuthEmailAccountCreation(ctx, user.ID, invitationCode); rollbackErr != nil {
			return infraerrors.InternalServer(
				"SUPPLIER_SIGNUP_ROLLBACK_FAILED",
				"failed to rollback supplier signup",
			).WithCause(rollbackErr)
		}
		return err
	}
	user.SupplierStatus = service.SupplierStatusPending
	user.HasSupplierAccess = false
...
func enrichSupplierAccess(ctx context.Context, supplierService *service.SupplierService, user *service.User) error {
	if supplierService == nil || user == nil {
		return nil
	}
	profile, allowed, err := supplierService.GetApprovedProfileByUserID(ctx, user.ID)
	if err != nil {
		return err
	}
