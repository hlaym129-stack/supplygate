...
func TestResolvePageImagePath(t *testing.T) {
	...
	want := mustEvalPageImagePathSymlinks(t, filepath.Join(base, "logo.png"))
	if got != want {
	...
	want = mustEvalPageImagePathSymlinks(t, filepath.Join(base, "images", "logo.png"))
	if got != want {
		t.Fatalf("path = %q, want %q", got, want)
	}
...
func mustEvalPageImagePathSymlinks(t *testing.T, path string) string {
	t.Helper()
