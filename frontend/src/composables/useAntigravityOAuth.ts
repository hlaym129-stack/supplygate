...
export function useAntigravityOAuth(options: AntigravityOAuthOptions = {}) {
  ...
  const generateAuthUrl = async (proxyId: number | null | undefined): Promise<boolean> => {
    ...
    try {
      ...
      const response = options.authUrlEndpoint
        ? (await apiClient.post(options.authUrlEndpoint, payload)).data
  ...
  const exchangeAuthCode = async (params: {
    ...
    try {
      ...
      const tokenInfo = options.exchangeCodeEndpoint
        ? (await apiClient.post(options.exchangeCodeEndpoint, payload)).data
  ...
  const validateRefreshToken = async (
    ...
    try {
      const tokenInfo = options.refreshTokenEndpoint
        ? (await apiClient.post(options.refreshTokenEndpoint, {
