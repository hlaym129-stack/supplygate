...
func (r *userRepository) ListWithFilters(ctx context.Context, params pagination.PaginationParams, filters service.UserListFilters) ([]service.User, *pagination.PaginationResult, error) {
	...
	switch strings.TrimSpace(strings.ToLower(filters.UserType)) {
	case "supplier":
		q = q.Where(dbuser.HasSupplierProfile())
	case "regular":
		q = q.Where(dbuser.RoleNEQ(service.RoleAdmin), dbuser.Not(dbuser.HasSupplierProfile()))
	}
	if filters.Search != "" {
		q = q.Where(
			dbuser.Or(
	...
	if err := r.loadSupplierProfileSummaries(ctx, userMap, userIDs); err != nil {
		return nil, nil, err
	}
...
func (r *userRepository) loadSupplierProfileSummaries(ctx context.Context, userMap map[int64]*service.User, userIDs []int64) error {
	if len(userIDs) == 0 {
		return nil
	}
	profiles, err := r.client.SupplierProfile.Query().
		Where(dbsupplier.UserIDIn(userIDs...)).
		All(ctx)
	if err != nil {
		return err
	}
	for _, profile := range profiles {
		u, ok := userMap[profile.UserID]
		if !ok {
			continue
		}
	...
	for _, u := range userMap {
		switch {
		case u.Role == service.RoleAdmin:
			u.UserType = "admin"
		case u.SupplierStatus != "":
			u.UserType = "supplier"
		default:
			u.UserType = "regular"
		}
