...
func (Account) Fields() []ent.Field {
	return []ent.Field{
		...
		field.String("owner_type").
			MaxLen(20).
			Default(domain.AccountOwnerTypePlatform),
		field.Int64("supplier_id").
			Optional().
			Nillable(),
		field.String("approval_status").
			MaxLen(20).
			Default(domain.AccountApprovalStatusApproved),
		...
		field.JSON("supported_models", []string{}).
			...
			SchemaType(map[string]string{dialect.Postgres: "jsonb"}),
		field.Time("supplier_tested_at").
			Optional().
			Nillable().
			SchemaType(map[string]string{dialect.Postgres: "timestamptz"}),
		field.JSON("supplier_tested_models", []string{}).
			Default(func() []string { return []string{} }).
			SchemaType(map[string]string{dialect.Postgres: "jsonb"}),
		field.String("supplier_edit_request_status").
			Optional().
			Nillable().
			MaxLen(20),
		field.String("supplier_edit_request_reason").
			Optional().
			Nillable().
			SchemaType(map[string]string{dialect.Postgres: "text"}),
		field.String("supplier_edit_request_review_note").
			Optional().
			Nillable().
			SchemaType(map[string]string{dialect.Postgres: "text"}),
		field.Time("supplier_edit_requested_at").
			Optional().
			Nillable().
			SchemaType(map[string]string{dialect.Postgres: "timestamptz"}),
		field.Time("supplier_edit_request_reviewed_at").
			Optional().
			Nillable().
			SchemaType(map[string]string{dialect.Postgres: "timestamptz"}),
		field.Int64("supplier_edit_request_reviewed_by").
			Optional().
			Nillable(),
...
func (Account) Edges() []ent.Edge {
	return []ent.Edge{
		...
		edge.From("supplier", SupplierProfile.Type).
			Ref("accounts").
			Field("supplier_id").
			Unique(),
	}
...
func (Account) Indexes() []ent.Index {
	return []ent.Index{
		...
		index.Fields("owner_type"),      // 筛选平台/供应商账号
		index.Fields("supplier_id"),     // 供应商账号列表
		index.Fields("approval_status"), // 审核状态筛选
		index.Fields("supplier_edit_request_status"),
