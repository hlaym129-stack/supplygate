...
func prepareUsageLogInsert(log *service.UsageLog) usageLogInsertPrepared {
	...
	return usageLogInsertPrepared{
		...
		args: []any{
			...
			log.AccountID,
			log.SupplierID,
			usageLogOwnerType(log.AccountOwnerType),
			requestIDArg,
			log.Model,
...
func usageLogOwnerType(value string) string {
...
func scanUsageLog(scanner interface{ Scan(...any) error }) (*service.UsageLog, error) {
	var (
		...
		accountOwnerType      sql.NullString
	...
	log := &service.UsageLog{
		...
		APIKeyID:              apiKeyID,
		AccountID:             accountID,
		SupplierID:            nullInt64Ptr(supplierID),
		AccountOwnerType:      coalesceTrimmedString(accountOwnerType, service.AccountOwnerTypePlatform),
		Model:                 model,
