...
func NewGatewayService(
	...
	resolver *ModelPricingResolver,
	balanceNotifyService *BalanceNotifyService,
	optionalSupplierRepo ...SupplierRepository,
) *GatewayService {
	var supplierRepo SupplierRepository
	if len(optionalSupplierRepo) > 0 {
		supplierRepo = optionalSupplierRepo[0]
	}
...
func (s *GatewayService) recordUsageCore(ctx context.Context, input *recordUsageCoreInput, opts *recordUsageOpts) error {
	...
	if apiKey.GroupID != nil {
		applyAccountStatsCost(ctx, usageLog, s.channelService, s.billingService, s.supplierRepo, account,
...
func (s *GatewayService) buildRecordUsageLog(
	...
	usageLog := &UsageLog{
		...
		AccountOwnerType:      account.OwnerType,
		RequestID:             requestID,
		Model:                 result.Model,
...
func (s *GatewayService) initDebugGatewayBodyFile(path string) {
	...
	//nolint:gosec // path from debug config, not user-controlled
	...
	if dir := filepath.Dir(path); dir != "." {
		//nolint:gosec // dir derived from debug config path
