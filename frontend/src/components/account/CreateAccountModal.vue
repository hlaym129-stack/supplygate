<template>
  <BaseDialog
    :show="show"
    :title="props.title || t('admin.accounts.createAccount')"
    width="wide"
    @close="handleClose"
  >
    ...
    >
      ...
      <div v-if="!isSupplierMode && form.platform === 'antigravity'" class="border-t border-gray-200 pt-4 dark:border-dark-600">
      ...
      <div v-if="form.type === 'apikey' && form.platform !== 'antigravity'" class="space-y-4">
        ...
        <div v-if="!isSupplierMode" class="border-t border-gray-200 pt-4 dark:border-dark-600">
          <label class="input-label">{{ t('admin.accounts.modelRestriction') }}</label>
      ...
      <div
        v-if="!isSupplierMode && form.platform === 'openai' && accountCategory === 'oauth-based'"
      ...
      <div v-if="!isSupplierMode" class="border-t border-gray-200 pt-4 dark:border-dark-600 space-y-4">
        <div class="mb-3 flex items-center justify-between">
          <div>
      ...
      >
        ...
        <!-- Window Cost Limit -->
        <div v-if="!isSupplierMode" class="rounded-lg border border-gray-200 p-4 dark:border-dark-600">
          <div class="mb-3 flex items-center justify-between">
            <div>
      ...
      <div v-if="isSupplierMode" class="rounded-lg border border-amber-200 bg-amber-50 p-4 dark:border-amber-900/50 dark:bg-amber-950/20">
        <div class="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
          <div>
            <label class="input-label mb-1">支持模型</label>
            <p class="text-xs text-amber-700 dark:text-amber-300">请选择该上游账号可提供的模型；提交前系统会逐个测试，全部通过后才会进入审核。</p>
          </div>
          <button type="button" class="btn btn-secondary btn-sm" :disabled="supplierModelsLoading" @click="loadSupplierDefaultModels">
            {{ supplierModelsLoading ? '加载中' : '刷新模型' }}
          </button>
        </div>
        <div class="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
          <label v-for="model in supplierAvailableModels" :key="model" class="flex items-center gap-2 rounded-md border border-amber-200 bg-white px-3 py-2 text-sm dark:border-amber-900/50 dark:bg-dark-800">
            <input v-model="supplierSupportedModels" type="checkbox" :value="model" class="rounded border-gray-300 text-primary-600" />
            <span class="truncate">{{ model }}</span>
          </label>
        </div>
        <div class="mt-3 flex gap-2">
          <input v-model="supplierCustomModel" type="text" class="input" placeholder="手动添加模型 ID，例如 claude-sonnet-4-5" @keydown.enter.prevent="addSupplierCustomModel" />
          <button type="button" class="btn btn-secondary" @click="addSupplierCustomModel">添加</button>
        </div>
        <div v-if="supplierSupportedModels.length" class="mt-3 flex flex-wrap gap-2">
          <span v-for="model in supplierSupportedModels" :key="model" class="inline-flex items-center gap-1 rounded-full bg-amber-100 px-2.5 py-1 text-xs text-amber-800 dark:bg-amber-900/40 dark:text-amber-200">
            {{ model }}
            <button type="button" class="font-semibold" @click="removeSupplierModel(model)">×</button>
          </span>
        </div>
        <p v-if="supplierLastTestedAt" class="mt-2 text-xs text-green-700 dark:text-green-300">已测试通过：{{ supplierLastTestedAt }}</p>
      </div>

      <div v-if="isSupplierMode" class="rounded-lg border border-emerald-200 bg-emerald-50 p-4 dark:border-emerald-900/50 dark:bg-emerald-950/20">
        <div class="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
          <div>
            <label class="input-label mb-1">结算报价</label>
            <p class="text-xs text-emerald-700 dark:text-emerald-300">按每百万 Token 填写供应商结算报价。报价只影响供应商结算，不影响用户扣费。</p>
          </div>
          <button type="button" class="btn btn-secondary btn-sm" :disabled="supplierPricingLoading" @click="() => prefillSupplierPricing()">
            {{ supplierPricingLoading ? '加载中' : '参考价预填' }}
          </button>
        </div>
        <div v-if="supplierPricingRows.length" class="mt-3 overflow-x-auto">
          <table class="min-w-full text-sm">
            ...
            <tbody class="divide-y divide-emerald-100 dark:divide-emerald-900/40">
              <tr v-for="row in supplierPricingRows" :key="row.model">
                <td class="py-2 pr-3 font-medium text-gray-900 dark:text-white">{{ row.model }}</td>
                <td class="py-2 px-2"><input v-model.number="row.input_mtok" type="number" min="0" step="0.0001" class="input" /></td>
                <td class="py-2 px-2"><input v-model.number="row.output_mtok" type="number" min="0" step="0.0001" class="input" /></td>
                <td class="py-2 px-2"><input v-model.number="row.cache_write_mtok" type="number" min="0" step="0.0001" class="input" /></td>
                <td class="py-2 px-2"><input v-model.number="row.cache_read_mtok" type="number" min="0" step="0.0001" class="input" /></td>
                <td class="py-2 pl-2"><input v-model.number="row.image_output_mtok" type="number" min="0" step="0.0001" class="input" /></td>
              </tr>
            </tbody>
          </table>
        </div>
        <p v-else class="mt-3 text-xs text-gray-500 dark:text-gray-400">请先选择支持模型，系统会为每个模型生成报价行。</p>
        <div class="mt-3">
          <label class="input-label text-xs">提交说明</label>
          <textarea v-model="supplierPricingSubmitNote" class="input min-h-[72px]" placeholder="可填写报价依据、套餐说明或补充备注" />
        </div>
      </div>
      ...
      <div
        v-if="!isSupplierMode && form.platform === 'openai' && (accountCategory === 'oauth-based' || accountCategory === 'apikey')"
        class="border-t border-gray-200 pt-4 dark:border-dark-600"
      >
        <div class="flex items-center justify-between">
          <div>
      ...
      <div class="border-t border-gray-200 pt-4 dark:border-dark-600">
        ...
        <div v-if="!isSupplierMode && form.platform === 'antigravity'" class="flex items-center gap-2">
        ...
        <div v-if="!isSupplierMode && form.platform === 'antigravity'" class="mt-3 flex items-center gap-2">
          <label class="flex cursor-pointer items-center gap-2">
            <input
              type="checkbox"
        ...
        <GroupSelector
          v-if="!isSupplierMode && !authStore.isSimpleMode"
        ...
        >
          ...
          {{
            isOAuthFlow
              ? t('common.next')
              : submitting
                ? (isSupplierMode ? '测试并提交中' : t('admin.accounts.creating'))
                : isEditing
                  ? '重新提交'
                  : t('common.create')
          }}
...
interface Props {
  ...
  groups: AdminGroup[]
  mode?: 'admin' | 'supplier'
  createAccount?: (payload: CreateAccountRequest) => Promise<unknown>
  updateAccount?: (id: number, payload: CreateAccountRequest) => Promise<unknown>
...
const isSupplierMode = computed(() => props.mode === 'supplier')
const isEditing = computed(() => !!props.initialAccount?.id)
const supplierAccountOAuthEndpoints = {
  generateAuthUrl: '/supplier/accounts/generate-auth-url',
  generateSetupTokenUrl: '/supplier/accounts/generate-setup-token-url',
  exchangeCode: '/supplier/accounts/exchange-code',
  exchangeSetupTokenCode: '/supplier/accounts/exchange-setup-token-code',
  cookieAuth: '/supplier/accounts/cookie-auth',
  setupTokenCookieAuth: '/supplier/accounts/setup-token-cookie-auth'
}

// OAuth composables
const oauth = useAccountOAuth(isSupplierMode.value ? supplierAccountOAuthEndpoints : {}) // For Anthropic OAuth
const openaiOAuth = useOpenAIOAuth(isSupplierMode.value ? { endpointPrefix: '/supplier/openai' } : {}) // For OpenAI OAuth
const geminiOAuth = useGeminiOAuth(isSupplierMode.value ? {
  authUrlEndpoint: '/supplier/gemini/oauth/auth-url',
  exchangeCodeEndpoint: '/supplier/gemini/oauth/exchange-code',
  capabilitiesEndpoint: '/supplier/gemini/oauth/capabilities'
} : {}) // For Gemini OAuth
const antigravityOAuth = useAntigravityOAuth(isSupplierMode.value ? {
  authUrlEndpoint: '/supplier/antigravity/oauth/auth-url',
  exchangeCodeEndpoint: '/supplier/antigravity/oauth/exchange-code',
  refreshTokenEndpoint: '/supplier/antigravity/oauth/refresh-token'
...
const supplierAvailableModels = ref<string[]>([])
const supplierSupportedModels = ref<string[]>([])
const supplierCustomModel = ref('')
const supplierModelsLoading = ref(false)
const supplierPricingLoading = ref(false)
const supplierPricingRows = ref<SupplierPricingRow[]>([])
const supplierPricingSubmitNote = ref('')
const supplierTestToken = ref('')
const supplierLastTestedAt = ref('')
...
// Load global feature states once. Supplier mode must not call admin-only APIs.
if (!isSupplierMode.value) {
...
watch(
  ...
  (newVal) => {
    if (newVal) {
      ...
      // Load TLS fingerprint profiles only for admin mode.
      if (!isSupplierMode.value) {
...
watch(
  ...
  () => {
    supplierTestToken.value = ''
    supplierLastTestedAt.value = ''
    if (isSupplierMode.value && props.show) {
      supplierSupportedModels.value = []
      loadSupplierDefaultModels()
    }
  }
)
...
watch(
  supplierSupportedModels,
  async () => {
    supplierTestToken.value = ''
    supplierLastTestedAt.value = ''
    const previousRows = supplierPricingRows.value.map(row => ({ ...row }))
    syncSupplierPricingRows()
    const hasNewEmptyRows = supplierPricingRows.value.some(row => {
      const previous = previousRows.find(item => item.model === row.model)
      if (previous) return false
      return ![
        row.input_mtok,
        row.output_mtok,
        row.cache_write_mtok,
        row.cache_read_mtok,
        row.image_output_mtok
      ].some(value => value != null && Number(value) > 0)
    })
    if (hasNewEmptyRows) {
      await prefillSupplierPricing({ silent: true })
    }
  },
  { deep: true }
)
...
const stripSupplierRestrictedAccountFields = (payload: CreateAccountRequest): CreateAccountRequest => {
  if (!isSupplierMode.value) {
    return payload
  }
...
const normalizeSupplierModels = (models: string[]) => {
...
const loadSupplierDefaultModels = async () => {
  if (!isSupplierMode.value) return
  supplierModelsLoading.value = true
  try {
    supplierAvailableModels.value = await supplierAPI.listModels(form.platform, form.type)
    if (supplierSupportedModels.value.length === 0 && supplierAvailableModels.value.length > 0) {
      supplierSupportedModels.value = supplierAvailableModels.value.slice(0, 1)
    }
  } catch (error: any) {
    appStore.showError(error.message || '加载模型列表失败')
  } finally {
    supplierModelsLoading.value = false
  }
}

const addSupplierCustomModel = () => {
  const model = supplierCustomModel.value.trim()
  if (!model) return
  supplierAvailableModels.value = normalizeSupplierModels([...supplierAvailableModels.value, model])
  supplierSupportedModels.value = normalizeSupplierModels([...supplierSupportedModels.value, model])
  supplierCustomModel.value = ''
}

const removeSupplierModel = (model: string) => {
  supplierSupportedModels.value = supplierSupportedModels.value.filter(item => item !== model)
}

const mtokToPerToken = (value: number | null | undefined) => {
  if (value == null || !Number.isFinite(Number(value)) || Number(value) <= 0) return null
  return Number(value) / 1_000_000
}

const perTokenToMTok = (value: number | null | undefined) => {
  if (value == null || !Number.isFinite(Number(value)) || Number(value) <= 0) return null
  return Number((Number(value) * 1_000_000).toFixed(6))
}

const syncSupplierPricingRows = () => {
  if (!isSupplierMode.value) return
  const current = new Map(supplierPricingRows.value.map(row => [row.model, row]))
  supplierPricingRows.value = normalizeSupplierModels(supplierSupportedModels.value).map(model => current.get(model) || {
    model,
    input_mtok: null,
    output_mtok: null,
    cache_write_mtok: null,
    cache_read_mtok: null,
    image_output_mtok: null
  })
}

const fillSupplierPricingRows = (defaults: ChannelModelPricing[]) => {
  supplierPricingRows.value = supplierPricingRows.value.map((row, index) => {
    const pricing = defaults[index]
    return {
      ...row,
      input_mtok: row.input_mtok ?? perTokenToMTok(pricing.input_price),
      output_mtok: row.output_mtok ?? perTokenToMTok(pricing.output_price),
      cache_write_mtok: row.cache_write_mtok ?? perTokenToMTok(pricing.cache_write_price),
      cache_read_mtok: row.cache_read_mtok ?? perTokenToMTok(pricing.cache_read_price),
      image_output_mtok: row.image_output_mtok ?? perTokenToMTok(pricing.image_output_price)
    }
  })
}

const prefillSupplierPricing = async (options?: { silent?: boolean }) => {
  if (!isSupplierMode.value) return
  syncSupplierPricingRows()
  if (supplierPricingRows.value.length === 0) {
    if (!options?.silent) {
      appStore.showError('请先选择支持模型')
    }
    return
  }
  supplierPricingLoading.value = true
  try {
    const defaults = await Promise.all(supplierPricingRows.value.map(row => supplierAPI.modelPricing(row.model)))
    fillSupplierPricingRows(defaults)
    if (!options?.silent) {
      appStore.showSuccess('参考价已预填')
    }
  } catch (error: any) {
    const msg = error?.response?.data?.message || error?.message || '加载模型参考价失败'
    if (!options?.silent) {
      appStore.showError(msg)
    }
  } finally {
    supplierPricingLoading.value = false
  }
}

const buildSupplierSettlementPricing = () => {
  syncSupplierPricingRows()
  const supportedModels = normalizeSupplierModels(supplierSupportedModels.value)
  if (supplierPricingRows.value.length !== supportedModels.length) {
    throw new Error('结算报价必须覆盖全部支持模型')
  }
  return supplierPricingRows.value.map(row => {
    const prices = [
      row.input_mtok,
      row.output_mtok,
      row.cache_write_mtok,
    ...
    if (prices.some(value => value < 0)) {
      throw new Error('结算报价不能为负数')
    }
    if (!prices.some(value => value > 0)) {
      throw new Error(`模型 ${row.model} 至少需要填写一个有效报价`)
    }
    return {
      platform: form.platform,
      models: [row.model],
...
const ensureSupplierPretestPayload = async (payload: CreateAccountRequest): Promise<CreateAccountRequest> => {
  if (!isSupplierMode.value) return payload
  const supportedModels = normalizeSupplierModels(supplierSupportedModels.value)
  if (supportedModels.length === 0) {
    throw new Error('请至少选择一个支持模型')
  }
  const basePayload = stripSupplierRestrictedAccountFields({
    ...payload,
    supported_models: supportedModels
  } as CreateAccountRequest) as CreateAccountRequest & { supported_models?: string[]; test_token?: string }
  const testResult = isEditing.value && props.initialAccount?.id
    ? await supplierAPI.testAccountUpdate(props.initialAccount.id, basePayload)
    : await supplierAPI.testAccount(basePayload)
  supplierTestToken.value = testResult.test_token
  supplierLastTestedAt.value = new Date(testResult.tested_at).toLocaleString()
  return {
    ...basePayload,
    supported_models: testResult.models,
    test_token: testResult.test_token,
    settlement_pricing: buildSupplierSettlementPricing(),
    submit_note: supplierPricingSubmitNote.value.trim()
  } as CreateAccountRequest
}

const supplierSafeProxyID = () => isSupplierMode.value ? null : form.proxy_id
...
const submitCreateAccount = async (payload: CreateAccountRequest) => {
  ...
  try {
    const finalPayload = await ensureSupplierPretestPayload(payload)
    await createAccountRequest(finalPayload)
    appStore.showSuccess(props.successMessage || t('admin.accounts.accountCreated'))
    emit('created')
    handleClose()
  } catch (error: any) {
...
const createAccountRequest = async (payload: CreateAccountRequest) => {
  const requestPayload = stripSupplierRestrictedAccountFields(payload)
  if (isEditing.value && props.initialAccount?.id && props.updateAccount) {
    await props.updateAccount(props.initialAccount.id, requestPayload)
    return
  }
  if (props.createAccount) {
    await props.createAccount(requestPayload)
    return
  }
  await adminAPI.accounts.create(withAntigravityConfirmFlag(requestPayload))
}

const anthropicExchangeEndpoint = () => {
  if (isSupplierMode.value) {
    return addMethod.value === 'oauth'
      ? '/supplier/accounts/exchange-code'
      : '/supplier/accounts/exchange-setup-token-code'
  }
  return addMethod.value === 'oauth'
...
const anthropicCookieAuthEndpoint = () => {
  if (isSupplierMode.value) {
    return addMethod.value === 'oauth'
      ? '/supplier/accounts/cookie-auth'
      : '/supplier/accounts/setup-token-cookie-auth'
  }
  return addMethod.value === 'oauth'
...
const resetForm = () => {
  ...
  supplierAvailableModels.value = []
  supplierSupportedModels.value = []
  supplierCustomModel.value = ''
  supplierPricingRows.value = []
  supplierPricingSubmitNote.value = ''
  supplierTestToken.value = ''
  supplierLastTestedAt.value = ''
  accountCategory.value = 'oauth-based'
...
const applyInitialAccount = (account: Account) => {
  form.name = account.name || ''
  form.notes = account.notes || ''
  form.platform = account.platform
  form.type = account.type
  form.credentials = { ...(account.credentials || {}) }
  form.proxy_id = isSupplierMode.value ? null : (account.proxy_id ?? null)
  ...
  form.group_ids = isSupplierMode.value ? [] : (account.group_ids || account.groups?.map(group => group.id) || [])
  supplierSupportedModels.value = [...(account.supported_models || [])]
  supplierAvailableModels.value = normalizeSupplierModels([...(account.supported_models || []), ...supplierAvailableModels.value])
  syncSupplierPricingRows()
  if (isSupplierMode.value && account.settlement_pricing?.length) {
    const pricingByModel = new Map<string, ChannelModelPricing>()
    account.settlement_pricing.forEach(pricing => {
      pricing.models.forEach(model => pricingByModel.set(model, pricing))
    })
    supplierPricingRows.value = supplierPricingRows.value.map(row => {
  ...
  autoPauseOnExpired.value = account.auto_pause_on_expired !== false
  ...
  if (account.type === 'apikey') {
    apiKeyBaseUrl.value = String(credentials.base_url || apiKeyBaseUrl.value)
    apiKeyValue.value = String(credentials.api_key || '')
    if (account.platform === 'antigravity') {
      antigravityAccountType.value = 'upstream'
      upstreamBaseUrl.value = String(credentials.base_url || '')
      upstreamApiKey.value = String(credentials.api_key || '')
    }
  }
  if (account.type === 'bedrock') {
    ...
    bedrockSecretAccessKey.value = String(credentials.aws_secret_access_key || '')
    bedrockSessionToken.value = String(credentials.aws_session_token || '')
    bedrockApiKeyValue.value = String(credentials.api_key || '')
    bedrockForceGlobal.value = credentials.aws_force_global === true || credentials.aws_force_global === 'true'
  }
  if (account.type === 'service_account') {
    vertexServiceAccountJson.value = String(credentials.service_account_json || '')
    vertexProjectId.value = String(credentials.project_id || '')
    vertexClientEmail.value = String(credentials.client_email || '')
    vertexLocation.value = String(credentials.location || 'global')
  }
}
...
const handleGenerateUrl = async () => {
  const proxyID = supplierSafeProxyID()
  if (form.platform === 'openai') {
...
const handleOpenAIExchange = async (authCode: string) => {
  ...
  try {
    ...
    if (shouldCreateOpenAI) {
      await createAccountRequest(await ensureSupplierPretestPayload({
        name: form.name,
        notes: form.notes,
        platform: 'openai',
        type: 'oauth',
  ...
  try {
    for (let i = 0; i < refreshTokens.length; i++) {
      try {
        ...
        await createAccountRequest(await ensureSupplierPretestPayload(createPayload))
        successCount++
      } catch (error: any) {
        failedCount++
        const errMsg = error.response?.data?.detail || error.message || 'Unknown error'
...
const handleAnthropicExchange = async (authCode: string) => {
  ...
  try {
    ...
    const endpoint = anthropicExchangeEndpoint()
...
const handleCookieAuth = async (sessionKey: string) => {
  ...
  try {
    ...
    const endpoint = anthropicCookieAuthEndpoint()
