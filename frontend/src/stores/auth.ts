...
export const useAuthStore = defineStore('auth', () => {
  ...
  function warnOnLegacySupplierRole(userData: Pick<User, 'role' | 'has_supplier_access' | 'email' | 'id'>): void {
    if (userData.role !== 'supplier') {
      return
    }

    console.warn(
      '[auth] /auth/me returned legacy supplier role. Admin accounts should use role=admin and express supplier capability via has_supplier_access.',
      {
        userId: userData.id,
        email: userData.email,
        role: userData.role,
        hasSupplierAccess: userData.has_supplier_access === true
      }
    )
  }
  ...
  function setAuthFromResponse(response: AuthResponse): void {
    ...
    warnOnLegacySupplierRole(userData)
    user.value = userData
