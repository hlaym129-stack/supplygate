...
function roleDashboard(authState: MockAuthState): string {
  ...
  if (hasSupplierProfile) return '/supplier/dashboard'
  return '/dashboard'
}
...
function simulateGuard(
  ...
  if (requiresSupplier && !authState.isSupplier) {
    return roleDashboard(authState)
  }
...
describe('路由守卫逻辑', () => {
  ...
  describe('管理员与供应商复合权限', () => {
    it('管理员且具备供应商权限时访问 /login 仍重定向到 /admin/dashboard', () => {
      const authState: MockAuthState = {
        isAuthenticated: true,
        isAdmin: true,
        isSupplier: true,
    ...
    it('纯供应商访问 /login 重定向到 /supplier/dashboard', () => {
      const authState: MockAuthState = {
        isAuthenticated: true,
        isAdmin: false,
        isSupplier: true,
      ...
      const redirect = simulateGuard('/login', { requiresAuth: false }, authState)
      expect(redirect).toBe('/supplier/dashboard')
    })

    it('纯供应商访问管理员页面被重定向到 /supplier/dashboard', () => {
      const authState: MockAuthState = {
        isAuthenticated: true,
        isAdmin: false,
        isSupplier: true,
    ...
    it('纯供应商访问 /dashboard 重定向到 /supplier/dashboard', () => {
      const authState: MockAuthState = {
        isAuthenticated: true,
        isAdmin: false,
        isSupplier: true,
    ...
    it('管理员兼供应商访问 /dashboard 不被强制切到供应商看板', () => {
      const authState: MockAuthState = {
        isAuthenticated: true,
        isAdmin: true,
        isSupplier: true,
