...
func (_q *UserQuery) QuerySupplierProfile() *SupplierProfileQuery {
...
func (_q *UserQuery) WithSupplierProfile(opts ...func(*SupplierProfileQuery)) *UserQuery {
	query := (&SupplierProfileClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withSupplierProfile; query != nil {
		if err := _q.loadSupplierProfile(ctx, query, nodes, nil,
			func(n *User, e *SupplierProfile) { n.Edges.SupplierProfile = e }); err != nil {
			return nil, err
		}
	}
...
func (_q *UserQuery) loadSupplierProfile(ctx context.Context, query *SupplierProfileQuery, nodes []*User, init func(*User), assign func(*User, *SupplierProfile)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(supplierprofile.FieldUserID)
	}
	query.Where(predicate.SupplierProfile(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.SupplierProfileColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
